...
function dumpProduction(){
    $(which mysqldump) --single-transaction --no-create-db --quick ${DB_PRODUCTION} > ${CURRENT_PATH};
}
...
function createSnapshot(){
    $(which mysqldump) --single-transaction --no-create-db --quick ${DB_STAGING} > ${SNAPSHOT_PATH};
}
